// projects.js — muestra y permite descargar los proyectos guardados localmente

document.addEventListener('DOMContentLoaded', function () {
  const list = document.getElementById('projectsList');
  const data = localStorage.getItem('saved_projects');
  if (!data) {
    list.innerHTML = '<p>No hay proyectos guardados.</p>';
    return;
  }
  try {
    ...
    window.download = function (i) {
      const arr = JSON.parse(localStorage.getItem('saved_projects')||'[]');
